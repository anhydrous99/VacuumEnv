...
Environment::Environment(int n, float dirty_percentage) : Environment(n) {
  ASSERT(0.0 <= dirty_percentage && dirty_percentage <= 1.0, "dirty percentage can only be between 0.0 and 1.0")
...
Environment::Environment(int n, float dirty_percentage, float percentage_obstacle) : Environment(n, dirty_percentage) {
  ASSERT(0.0 <= percentage_obstacle && percentage_obstacle <= 1.0, "percentage obstacle can only between 0.0 and 1.0")
  unsigned long n_elements = _data.size();
...
cell &Environment::operator()(int i, int j) {
  ASSERT(0 <= i && i < _n, "Input i index out of range")
  ASSERT(0 <= j && j < _n, "Input j index out of range")
  return _data[i * _n + j];
}
...
cell &Environment::operator[](int i) {
  ASSERT(0 <= i && i < _n * _n, "Out of bounds")
  return _data[i];
}
...
std::ostream &operator<<(std::ostream &os, const Environment &environment) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    for (int j = 0; j < n; j++) {
      ...
      os << environment._data[current_row + j] << ((found) ? "<" : " ");
    }
    os << std::endl;
  }
  return os;
